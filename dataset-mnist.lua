...
function mnist.loadDataset(fileName, maxLoad)
   ...
   function dataset:binary(subclass,geometry)
      size = 0 
      for i=1,subclass:size(1) do 
        size = size + labels[labels:eq(subclass[i])]:size(1)
      end
      ndata = torch.Tensor(size,geometry[1],geometry[2],geometry[3]) 
      nlabel = torch.Tensor(size)
      ii = 1 
      for i=1,subclass:size(1) do 
         inds = labels:eq(subclass[i])
