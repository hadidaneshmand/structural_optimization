...
function noisy_train(dataset,model_in,opt_in,epoch_in, noisy_itrs, noise_var)
   -- epoch tracker
   ...
   for tt = 1, epoch do 
     ...
     for t = 1,dataset:size(),opt.batchSize do
        ...
        local feval = function(x)
           ...
           -- add noise: 
           add_noise = false
           for i=1,noisy_itrs:size(1) do 
             if tt > noisy_itrs[i] and tt < noisy_itrs[i] + 90 then 
               add_noise = true
             end
           end
           if add_noise and t == 1 then
              print('add_noise')
              gradParameters = gradParameters:add(torch.randn(gradParameters:size(1)):mul(noise_var))
           end
           -- update confusion
...
--   print('<trainer> saving network to '..filename)
   ...
   -- next epoch
   epoch = epoch + 1
   return losses,grads,itrs,grads_1,grads_2,grads_l
end